_NIL={}
function csclass(config)
  local cls=config.csextends or Object
  local name=config.name
  return setmetatable(config.csstatic or {},{
    __call=function(t,...)
      local args={...}
      local constructor=function(cssuper,...)
        if table.size(args)==1 and type(args[1])=="table" and luajava.instanceof(args[1][1],cls) then
          cssuper=function(...)
            return args[1][1]
          end
        end
        return (config.constructor or function(cssuper,...)
          return cssuper(...)
        end)(cssuper,...)
      end
      local fields={}
