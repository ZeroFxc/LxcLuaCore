...
function Observable:sample(sampler)
  if not sampler then error('需要一个Observable类型的采样器') end
...
function Observable:rxswitch()
  return Observable.create(function(observer)
    ...
    -- @nekolaska 
    -- Modified function name because AndroLua+ uses the switch keyword
    local function _switch(source)
      if innerSubscription then
        innerSubscription:unsubscribe()
      end
    ...
    local subscription = self:subscribe(_switch, onError, onCompleted)
    return Subscription.create(function()
      if innerSubscription then
        innerSubscription:unsubscribe()
      end
