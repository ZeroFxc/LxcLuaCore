...
int luaO_executeVM (lua_State *L, Proto *f) {
  ...
  while (pc < vm->size) {
    ...
    switch (lua_op) {
      ...
      case OP_ADDI: { TValue *rb = s2v(base + b); int imm = sC2int(c); if (ttispointer(rb)) { setptrvalue(s2v(base + a), (char *)ptrvalue(rb) + imm); pc++; } else if (ttisinteger(rb)) { setivalue(s2v(base + a), intop(+, ivalue(rb), (lua_Integer)imm)); pc++; } else if (tonumberns(rb, nb)) { setfltvalue(s2v(base + a), luai_numadd(L, nb, cast_num(imm))); pc++; } break; }
      case OP_ADDK: { TValue *rb = s2v(base + b); TValue *rc = k + c; if (ttispointer(rb) && ttisinteger(rc)) { setptrvalue(s2v(base + a), (char *)ptrvalue(rb) + ivalue(rc)); pc++; } else if (ttisinteger(rb) && ttisinteger(rc)) { setivalue(s2v(base + a), intop(+, ivalue(rb), ivalue(rc))); pc++; } else if (tonumberns(rb, nb) && tonumberns(rc, nc)) { setfltvalue(s2v(base + a), luai_numadd(L, nb, nc)); pc++; } break; }
      case OP_SUBK: { TValue *rb = s2v(base + b); TValue *rc = k + c; if (ttispointer(rb) && ttisinteger(rc)) { setptrvalue(s2v(base + a), (char *)ptrvalue(rb) - ivalue(rc)); pc++; } else if (ttisinteger(rb) && ttisinteger(rc)) { setivalue(s2v(base + a), intop(-, ivalue(rb), ivalue(rc))); pc++; } else if (tonumberns(rb, nb) && tonumberns(rc, nc)) { setfltvalue(s2v(base + a), luai_numsub(L, nb, nc)); pc++; } break; }
      case OP_MULK: { TValue *rb = s2v(base + b); TValue *rc = k + c; if (ttisinteger(rb) && ttisinteger(rc)) { setivalue(s2v(base + a), intop(*, ivalue(rb), ivalue(rc))); pc++; } else if (tonumberns(rb, nb) && tonumberns(rc, nc)) { setfltvalue(s2v(base + a), luai_nummul(L, nb, nc)); pc++; } break; }
      case OP_MODK: { TValue *rb = s2v(base + b); TValue *rc = k + c; if (ttisinteger(rb) && ttisinteger(rc)) { setivalue(s2v(base + a), luaV_mod(L, ivalue(rb), ivalue(rc))); pc++; } else if (tonumberns(rb, nb) && tonumberns(rc, nc)) { setfltvalue(s2v(base + a), luaV_modf(L, nb, nc)); pc++; } break; }
      case OP_POWK: { TValue *rb = s2v(base + b); TValue *rc = k + c; if (tonumberns(rb, nb) && tonumberns(rc, nc)) { setfltvalue(s2v(base + a), luai_numpow(L, nb, nc)); pc++; } break; }
      case OP_DIVK: { TValue *rb = s2v(base + b); TValue *rc = k + c; if (tonumberns(rb, nb) && tonumberns(rc, nc)) { setfltvalue(s2v(base + a), luai_numdiv(L, nb, nc)); pc++; } break; }
      case OP_IDIVK: { TValue *rb = s2v(base + b); TValue *rc = k + c; if (ttisinteger(rb) && ttisinteger(rc)) { setivalue(s2v(base + a), luaV_idiv(L, ivalue(rb), ivalue(rc))); pc++; } else if (tonumberns(rb, nb) && tonumberns(rc, nc)) { setfltvalue(s2v(base + a), luai_numidiv(L, nb, nc)); pc++; } break; }
      ...
      case OP_MUL: { TValue *rb = s2v(base + b); TValue *rc = s2v(base + c); if (ttisinteger(rb) && ttisinteger(rc)) { setivalue(s2v(base + a), intop(*, ivalue(rb), ivalue(rc))); pc++; } else if (tonumberns(rb, nb) && tonumberns(rc, nc)) { setfltvalue(s2v(base + a), luai_nummul(L, nb, nc)); pc++; } break; }
      case OP_MOD: { TValue *rb = s2v(base + b); TValue *rc = s2v(base + c); if (ttisinteger(rb) && ttisinteger(rc)) { setivalue(s2v(base + a), luaV_mod(L, ivalue(rb), ivalue(rc))); pc++; } else if (tonumberns(rb, nb) && tonumberns(rc, nc)) { setfltvalue(s2v(base + a), luaV_modf(L, nb, nc)); pc++; } break; }
      case OP_POW: { TValue *rb = s2v(base + b); TValue *rc = s2v(base + c); if (tonumberns(rb, nb) && tonumberns(rc, nc)) { setfltvalue(s2v(base + a), luai_numpow(L, nb, nc)); pc++; } break; }
      case OP_DIV: { TValue *rb = s2v(base + b); TValue *rc = s2v(base + c); if (tonumberns(rb, nb) && tonumberns(rc, nc)) { setfltvalue(s2v(base + a), luai_numdiv(L, nb, nc)); pc++; } break; }
      case OP_IDIV: { TValue *rb = s2v(base + b); TValue *rc = s2v(base + c); if (ttisinteger(rb) && ttisinteger(rc)) { setivalue(s2v(base + a), luaV_idiv(L, ivalue(rb), ivalue(rc))); pc++; } else if (tonumberns(rb, nb) && tonumberns(rc, nc)) { setfltvalue(s2v(base + a), luai_numidiv(L, nb, nc)); pc++; } break; }
      case OP_BANDK: { TValue *rb = s2v(base + b); TValue *rc = k + c; lua_Integer i1; if (tointegerns(rb, &i1)) { setivalue(s2v(base + a), intop(&, i1, ivalue(rc))); pc++; } break; }
      case OP_BORK: { TValue *rb = s2v(base + b); TValue *rc = k + c; lua_Integer i1; if (tointegerns(rb, &i1)) { setivalue(s2v(base + a), intop(|, i1, ivalue(rc))); pc++; } break; }
      case OP_BXORK: { TValue *rb = s2v(base + b); TValue *rc = k + c; lua_Integer i1; if (tointegerns(rb, &i1)) { setivalue(s2v(base + a), intop(^, i1, ivalue(rc))); pc++; } break; }
      case OP_SHLI: { TValue *rb = s2v(base + b); int ic = sC2int(c); lua_Integer ib; if (tointegerns(rb, &ib)) { setivalue(s2v(base + a), luaV_shiftl(ib, ic)); pc++; } break; }
      case OP_SHRI: { TValue *rb = s2v(base + b); int ic = sC2int(c); lua_Integer ib; if (tointegerns(rb, &ib)) { setivalue(s2v(base + a), luaV_shiftl(ib, -ic)); pc++; } break; }
      case OP_BAND: { TValue *rb = s2v(base + b); TValue *rc = s2v(base + c); lua_Integer i1, i2; if (tointegerns(rb, &i1) && tointegerns(rc, &i2)) { setivalue(s2v(base + a), intop(&, i1, i2)); pc++; } break; }
      case OP_BOR: { TValue *rb = s2v(base + b); TValue *rc = s2v(base + c); lua_Integer i1, i2; if (tointegerns(rb, &i1) && tointegerns(rc, &i2)) { setivalue(s2v(base + a), intop(|, i1, i2)); pc++; } break; }
      case OP_BXOR: { TValue *rb = s2v(base + b); TValue *rc = s2v(base + c); lua_Integer i1, i2; if (tointegerns(rb, &i1) && tointegerns(rc, &i2)) { setivalue(s2v(base + a), intop(^, i1, i2)); pc++; } break; }
      case OP_SHL: { TValue *rb = s2v(base + b); TValue *rc = s2v(base + c); lua_Integer i1, i2; if (tointegerns(rb, &i1) && tointegerns(rc, &i2)) { setivalue(s2v(base + a), luaV_shiftl(i1, i2)); pc++; } break; }
      case OP_SHR: { TValue *rb = s2v(base + b); TValue *rc = s2v(base + c); lua_Integer i1, i2; if (tointegerns(rb, &i1) && tointegerns(rc, &i2)) { setivalue(s2v(base + a), luaV_shiftl(i1, -i2)); pc++; } break; }
      ...
      case OP_VARARG: { int n = c - 1; int vatab = flags ? b : -1; ci->u.l.savedpc = (const Instruction *)(f->code + pc); luaT_getvarargs(L, ci, base + a, n); if (vatab >= 0) { /* handle vatab */ } break; }
