...
static Expr *parse_prec(Parser *p, int min_prec) {
    ...
    while (1) {
        ...
        int prec = prec_of(k);
        if (prec < min_prec) {
            /* 检查上下文关键字 'is' 运算符: expr is ClassName */
            if (k == TOK_IDENT && peek(p)->len == 2 &&
                memcmp(peek(p)->start, "is", 2) == 0 &&
                min_prec <= PREC_CMP) {
                int line = peek(p)->line;
                advance(p); /* is */
                if (peek(p)->kind != TOK_IDENT) {
                    set_error(p, "expected class name after 'is'");
                    return NULL;
                }
                const Token *cls = advance(p);
                Expr *n = expr_new(p->pool, EXPR_IS, line);
                n->as.is_expr.obj = lhs;
                n->as.is_expr.class_name = cls->start;
                n->as.is_expr.class_name_len = cls->len;
                lhs = n;
                continue;
            }
            break;
...
/* ============================================================
 * OOP 语句解析：class / interface / trait
 *
 * 语法：
 *   class ClassName [: ParentClass] [implements Iface1, ...] body end
 *   interface InterfaceName [: ParentIface, ...] body end
 *   trait TraitName body end
 *
 * class/interface/trait 是上下文关键字（lexer 将其作为 TOK_IDENT），
 * 在 parse_ident_stmt 中通过标识符名称检测。
 * ============================================================ */

/* 解析逗号分隔的标识符列表，存储到 pool 分配的输出数组中。
 * 返回标识符数量。出错时返回 -1 并设置 parser error。 */
static int parse_ident_list(Parser *p, const char ***out_names,
                            size_t **out_lens) {
    ItemBuf names;
    ib_init(&names, sizeof(const Token *));
    *(const Token **)ib_push(&names) = advance(p);
    ...
    const Token **toks = names.data;
    const char **nms = node_pool_alloc(p->pool, sizeof(char *) * n);
    size_t *lns = node_pool_alloc(p->pool, sizeof(size_t) * n);
    for (int i = 0; i < n; i++) {
        nms[i] = toks[i]->start;
        lns[i] = toks[i]->len;
    }
...
/* 解析 class 语句：
 *   class ClassName [: ParentClass] [implements Iface1, ...] body end */
static Stmt *parse_class_stmt(Parser *p) {
    int line = peek(p)->line;
    advance(p); /* class */

    if (peek(p)->kind != TOK_IDENT) {
        set_error(p, "expected class name after 'class'");
        return NULL;
    }
    const Token *cls_name = advance(p);

    /* 可选的 extends 子句：: ParentClass */
    const char *parent_name = NULL;
    size_t parent_len = 0;
    if (match(p, TOK_COLON)) {
        if (peek(p)->kind != TOK_IDENT) {
            set_error(p, "expected parent class name after ':'");
            return NULL;
        }
        const Token *pn = advance(p);
        parent_name = pn->start;
        parent_len = pn->len;
    }

    /* 可选的 implements 子句 */
    const char **impl_names = NULL;
    size_t *impl_lens = NULL;
    int n_impl = 0;
    if (peek(p)->kind == TOK_IDENT &&
        peek(p)->len == 10 &&
        memcmp(peek(p)->start, "implements", 10) == 0) {
        advance(p); /* implements */
        if (peek(p)->kind != TOK_IDENT) {
            set_error(p, "expected interface name after 'implements'");
            return NULL;
        }
        n_impl = parse_ident_list(p, &impl_names, &impl_lens);
        if (n_impl < 0) return NULL;
    }
    ...
    parse_block1(p, &body, TOK_KW_END);
    frame_rewind(cur_frame(p), mark);
    expect(p, TOK_KW_END, "end (of class)");
    if (!p->ok) return NULL;

    Stmt *s = stmt_new(p->pool, STMT_CLASS, line);
    s->as.class_stmt.name = cls_name->start;
    s->as.class_stmt.name_len = cls_name->len;
    s->as.class_stmt.parent_name = parent_name;
    s->as.class_stmt.parent_len = parent_len;
    s->as.class_stmt.implements_names = impl_names;
    s->as.class_stmt.implements_lens = impl_lens;
    s->as.class_stmt.n_implements = n_impl;
...
/* 解析 interface 语句：
 *   interface InterfaceName [: ParentIface, ...] body end */
static Stmt *parse_interface_stmt(Parser *p) {
    int line = peek(p)->line;
    advance(p); /* interface */

    if (peek(p)->kind != TOK_IDENT) {
        set_error(p, "expected interface name after 'interface'");
        return NULL;
    }
    const Token *iface_name = advance(p);

    /* 可选的 extends 子句：: ParentIface, ... */
    const char **ext_names = NULL;
    size_t *ext_lens = NULL;
    int n_ext = 0;
    if (match(p, TOK_COLON)) {
        if (peek(p)->kind != TOK_IDENT) {
            set_error(p, "expected interface name after ':'");
            return NULL;
        }
        n_ext = parse_ident_list(p, &ext_names, &ext_lens);
        if (n_ext < 0) return NULL;
    }
    ...
    parse_block1(p, &body, TOK_KW_END);
    frame_rewind(cur_frame(p), mark);
    expect(p, TOK_KW_END, "end (of interface)");
    if (!p->ok) return NULL;

    Stmt *s = stmt_new(p->pool, STMT_INTERFACE, line);
    s->as.interface_stmt.name = iface_name->start;
    s->as.interface_stmt.name_len = iface_name->len;
    s->as.interface_stmt.extends_names = ext_names;
    s->as.interface_stmt.extends_lens = ext_lens;
    s->as.interface_stmt.n_extends = n_ext;
    s->as.interface_stmt.body = body;
    return s;
}

/* 解析 trait 语句：
 *   trait TraitName body end */
static Stmt *parse_trait_stmt(Parser *p) {
    int line = peek(p)->line;
    advance(p); /* trait */

    if (peek(p)->kind != TOK_IDENT) {
        set_error(p, "expected trait name after 'trait'");
        return NULL;
    }
    const Token *trait_name = advance(p);
    ...
    parse_block1(p, &body, TOK_KW_END);
    frame_rewind(cur_frame(p), mark);
    expect(p, TOK_KW_END, "end (of trait)");
    if (!p->ok) return NULL;

    Stmt *s = stmt_new(p->pool, STMT_TRAIT, line);
    s->as.trait_stmt.name = trait_name->start;
    s->as.trait_stmt.name_len = trait_name->len;
    s->as.trait_stmt.body = body;
    return s;
}
...
static Stmt *parse_stmt(Parser *p) {
    ...
    case TOK_IDENT: {
        ...
        const Token *t = peek(p);
        /* class/interface/trait 上下文关键字：当后跟标识符时视为 OOP 声明 */
        if (t->len == 5 && memcmp(t->start, "class", 5) == 0) {
            if (peek_at(p, 1)->kind == TOK_IDENT)
                return parse_class_stmt(p);
        }
        if (t->len == 9 && memcmp(t->start, "interface", 9) == 0) {
            if (peek_at(p, 1)->kind == TOK_IDENT)
                return parse_interface_stmt(p);
        }
        if (t->len == 5 && memcmp(t->start, "trait", 5) == 0) {
            if (peek_at(p, 1)->kind == TOK_IDENT)
