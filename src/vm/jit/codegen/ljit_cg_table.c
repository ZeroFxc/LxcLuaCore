...
void ljit_cg_emit_gettable(void *node_ptr, void *ctx_ptr) {
    ...
    JIT_DBG(MOD_CG_TABLE, "GETTABLE: pc=%d, dest=R%d, src1=R%d, src2=R%d",
        node->original_pc, node->dest.v.reg, node->src1.v.reg, node->src2.v.reg);
...
/* map容器代码生成 */
void ljit_cg_emit_newmap(void *node_ptr, void *ctx_ptr) {
    ljit_ir_node_t *node = (ljit_ir_node_t *)node_ptr;
    ljit_ctx_t *ctx = (ljit_ctx_t *)ctx_ptr;
    struct sljit_compiler *compiler = (struct sljit_compiler *)ctx->compiler;
    if (!node || !ctx || !compiler) return;
    ...
    JIT_DBG(MOD_CG_TABLE, "NEWMAP: pc=%d, dest=R%d",
        node->original_pc, node->dest.v.reg);
    ...
    /* Call ljit_icall_newmap */
    sljit_emit_icall(compiler, SLJIT_CALL, SLJIT_ARGS2V(W, W), SLJIT_IMM, (sljit_sw)ljit_icall_newmap);
}

void ljit_cg_emit_getmap(void *node_ptr, void *ctx_ptr) {
    ljit_ir_node_t *node = (ljit_ir_node_t *)node_ptr;
    ljit_ctx_t *ctx = (ljit_ctx_t *)ctx_ptr;
    struct sljit_compiler *compiler = (struct sljit_compiler *)ctx->compiler;
    if (!node || !ctx || !compiler) return;
    ...
    JIT_DBG(MOD_CG_TABLE, "GETMAP: pc=%d, dest=R%d, src1=R%d, src2=R%d",
        node->original_pc, node->dest.v.reg, node->src1.v.reg, node->src2.v.reg);
    ...
    /* Call ljit_icall_getmap */
    sljit_emit_icall(compiler, SLJIT_CALL, SLJIT_ARGS4V(W, W, W, W), SLJIT_IMM, (sljit_sw)ljit_icall_getmap);
...
void ljit_cg_emit_setmap(void *node_ptr, void *ctx_ptr) {
    ljit_ir_node_t *node = (ljit_ir_node_t *)node_ptr;
    ljit_ctx_t *ctx = (ljit_ctx_t *)ctx_ptr;
    struct sljit_compiler *compiler = (struct sljit_compiler *)ctx->compiler;
    if (!node || !ctx || !compiler) return;
    ...
    JIT_DBG(MOD_CG_TABLE, "SETMAP: pc=%d, dest=R%d, src1=R%d, src2=R%d",
        node->original_pc, node->dest.v.reg, node->src1.v.reg, node->src2.v.reg);
    ...
    /* Call ljit_icall_setmap */
    sljit_emit_icall(compiler, SLJIT_CALL, SLJIT_ARGS4V(W, W, W, W), SLJIT_IMM, (sljit_sw)ljit_icall_setmap);
}
