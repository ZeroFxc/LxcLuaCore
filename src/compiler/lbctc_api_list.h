...
X(lua_checkoverride, void, (lua_State *L, int class_idx, const char *name))
...
X(lua_extendiface, void, (lua_State *L, int child_idx, int parent_idx))
...
/* 泛型与克隆 - 待实现：
X(lua_clone, int, (lua_State *L))
X(lua_specialize, int, (lua_State *L, int class_idx, int type_args_idx))
*/
