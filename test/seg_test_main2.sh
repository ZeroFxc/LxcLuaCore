...
for seg in "${SEGMENTS[@]}"; do
    ...
    # 先规范化两个输出文件
    sed 's/^[^:]*:[0-9]*: //' "$OUTDIR/${name}_lua.txt" | sort -t' ' -k1,1 > "$OUTDIR/${name}_lua_norm.txt" 2>/dev/null
    sed 's/^[^:]*:[0-9]*: //' "$OUTDIR/${name}_run.txt" | sort -t' ' -k1,1 > "$OUTDIR/${name}_run_norm.txt" 2>/dev/null
