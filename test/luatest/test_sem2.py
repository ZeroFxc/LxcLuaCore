...
def send(msg, wait=True):
    ...
    while True:
        ...
        if not ch: print("EOF header"); return None
        header += ch
        if header.endswith(b'\r\n\r\n') or header.endswith(b'\n\n'): break
    cl = 0
    for l in header.decode('ascii','ignore').split('\r\n'):
        if l.lower().startswith('content-length:'): cl = int(l.split(':')[1].strip())
    if cl <= 0: return None
    body = proc.stdout.read(cl)
    try: return json.loads(body)
    except: return body.decode('utf-8','ignore')
...
if resp:
    ...
    print("  semanticTokensProvider:", sem)
...
if resp:
    ...
    print(f"SemanticTokens: {len(data)} integers")
    if data:
        # Print first 10 entries as 5-tuples
        for i in range(0, min(10, len(data)), 5):
            tup = data[i:i+5]
            print(f"  tuple: {tup}")
else:
    print("SemanticTokens: NULL (server crash)")
