...
# preview2 组件：读取 WASI 环境变量数量（验证宿主资源访问）
wat = '''(component
...
'''
open('_env.wat', 'w', encoding='utf-8').write(wat)
r1 = subprocess.run([wt, 'parse', '_env.wat', '-o', '_env.wasm'], capture_output=True, text=True)
print('parse:', r1.returncode, (r1.stderr or '')[:200])
if r1.returncode == 0:
    r2 = subprocess.run([wt, 'validate', '_env.wasm'], capture_output=True, text=True)
