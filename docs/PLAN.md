# LXCLUA-NCore Project Development Plan
...
## 1. Project Overview and Strengths
...
*   **Rich Syntax Extensions:** Native support for OOP (classes, interfaces, visibility modifiers), Switch statements, Try-Catch, Arrow Functions, Pipeline Operators (`|>`), Optional Chaining (`?.`), Nullish Coalescing (`??`), Kotlin-style Infix Functions, and Shell-style conditional testing (`[ -f "file" ]`).
...
*   **Complexity of Integration:** Maintaining synchronization between the custom 64-bit `lparser.c`/`lvm.c`, bytecode-to-C generation (`lbctc.c`), and WebAssembly bindings (`lwasm3.c`) requires immense care. Any structural changes to the Lua stack or bytecode format risk breaking multiple downstream modules.
...
*   **Documentation and API Standardization:** Deeper modules (like the `ByteCode` API or `lexer` manipulation) lack comprehensive, standardized documentation detailing edge cases (e.g., handling interpolated strings `<interpstring>` vs raw strings `<rawstring>`).
*   **Test Coverage for Edge Cases:** Some modern syntax features (like deeply nested concepts/superstructs, or infix function chains) may lack exhaustive regression tests, especially when combined with heavy CFF obfuscation.
...
2.  **Strengthen Bytecode-to-C Obfuscation:**
...
3.  **Native JIT Compilation:**
    *   Explore true JIT compilation of Lua bytecode to native code, building upon a separate `jit` module (sljit-based), the bytecode-to-C pipeline, and WASM generation infrastructure.
4.  **Package Manager / Ecosystem:**
    *   Develop a lightweight package manager (akin to LuaRocks) tailored for LXCLUA-NCore, supporting pre-compiled bytecode-to-C/WASM modules and pure Lua AST transformers.
