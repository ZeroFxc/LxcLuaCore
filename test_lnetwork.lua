...
test("parseUrl full URL", function()
    local p = http.url.parse("https://user:pass@example.com:8443/path?q=v#frag")
...
test("parseUrl simple URL", function()
    local p = http.url.parse("http://127.0.0.1:8080/")
...
test("base64 encode/decode", function()
    local enc = http.base64.encode("Hello, World!")
    assert(enc == "SGVsbG8sIFdvcmxkIQ==")
    assert(http.base64.decode(enc) == "Hello, World!")
end)
test("base64 empty", function()
    assert(http.base64.encode("") == "")
end)
print("")
...
test("mime_type html", function()
    assert(string.find(http.mime.type("html"), "text/html", 1, true) == 1)
end)
test("mime_type json", function()
    assert(string.find(http.mime.type("json"), "application/json", 1, true) == 1)
end)
test("mime_type png", function()
    assert(http.mime.type("png") == "image/png")
end)
test("mime_type unknown", function()
    assert(http.mime.type("xyz123unknown") == "application/octet-stream")
end)
print("")
...
test("cookie_parse", function()
    local cookies = http.cookie.parse("a=1; b=2; c=3")
    assert(#cookies == 3)
    local m = {}
